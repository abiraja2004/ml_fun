...
def combine_feat(feat_names, feat_path_name):
    ...
    for run in range(1,config.n_runs+1):
        ...
        for fold in range(1,config.n_folds+1):
            ...
            ## load label
            # train
            info_train = pd.read_csv("%s/train.info" % (save_path), encoding=config.text_encoding)
            ## change it to zero-based for multi-classification in xgboost
            Y_train = info_train["median_relevance"] - 1 
            # valid               
            info_valid = pd.read_csv("%s/valid.info" % (save_path), encoding=config.text_encoding)
    ...
    Y_train = info_train["median_relevance"] - 1 
    # test               
    info_test = pd.read_csv("%s/test.info" % (save_path), encoding=config.text_encoding)
