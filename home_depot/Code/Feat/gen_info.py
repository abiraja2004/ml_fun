...
def gen_info(feat_path_name):
    ...
    dfTrain_original = pd.read_csv(config.original_train_data_path, encoding=config.text_encoding).fillna("")
    dfTest_original = pd.read_csv(config.original_test_data_path, encoding=config.text_encoding).fillna("")
    ...
    for run in range(config.n_runs):
        ...
        for fold, (validInd, trainInd) in enumerate(skf[run]):
            ...
            #############################
            dfTrain_original.iloc[trainInd].to_csv("%s/train.info" % path, index=False, header=True, encoding=config.text_encoding)
            dfTrain_original.iloc[validInd].to_csv("%s/valid.info" % path, index=False, header=True, encoding=config.text_encoding)
    print("Done.")
    ...
    ## info        
    dfTrain_original.to_csv("%s/%s/All/train.info" % (config.feat_folder, feat_path_name), index=False, header=True, encoding=config.text_encoding)
    dfTest_original.to_csv("%s/%s/All/test.info" % (config.feat_folder, feat_path_name), index=False, header=True, encoding=config.text_encoding)
